...
def get_teacher_model():
    ...
    for key, value in check_point.copy().items():
        if (not ("weight" in key)) and (not( "bias" in key)) or ("scaling" in key) or ("offset" in key) or "_W" in key or "_A" in key or "gamma" in key:
            check_point.pop(key)
    teacher_model.load_state_dict(check_point, strict = False)
...
def get_model(config):
    ...
    if args.quant_mode:
        ...
        if args.init_param:
            for key, value in check_point.copy().items():
                if (not ("weight" in key)) and (not( "bias" in key)) or ("scaling" in key) or ("offset" in key):
                    check_point.pop(key)
        ...
        if args.after_search_finetuning:
            ...
            model = QILBertForSequenceClassification(bert_config)
            for key, value in check_point.copy().items():
                if "pruning" in key or "clipping" in key :
                    print(key,value)
                    check_point[key] = torch.Tensor([value])
                    print(check_point[key])
            model.load_state_dict(check_point, strict = False)
            for name, p in model.named_parameters():
                if "pruning" in name or "clipping" in name:
                    p.requires_grad = False
        else:
            model.load_state_dict(check_point, strict = False)
            for name, p in model.named_parameters():
                if "clipping" in name or "pruning" in name:
                    print(p)
